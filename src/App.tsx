...
常に「相手役そのもの」として返答し、AIとしてのメタ的な発言は絶対にしないでください。態度や口調も特性に従ってください。`;
  ...
  const getRoleplayFeedback = async () => {
    ...
    try {
      ...
      const prompt = `あなたは心理関係の専門家です。以下のロールプレイ（対話シミュレーション）のログを読み、ユーザー（あなた）への具体的なフィードバックを行ってください。
1. 良かった点（言い回しや態度など）
2. 改善点（もっとこう言うと相手は受け入れやすい、など）
3. この相手と実際に話す前の心構え
...
${conversation}`;
      ...
      setRoleplayState(prev => ({ ...prev, messages: [...prev.messages, { id: Date.now().toString(), role: 'model', text: `【フィードバック】\n\n${response.text}` }], loading: false }));
    } catch (e) {
      console.error(e);
      setRoleplayState(prev => ({ ...prev, loading: false }));
    }
              ...
              <div className="flex justify-between items-center p-4 border-b border-slate-800 bg-slate-900/50">
                ...
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="text-xs text-slate-400 hover:text-emerald-400 px-2 py-1 rounded bg-slate-800 transition-colors"
                  >
                    最初から
                  </button>
                  <button 
                ...
                {roleplayState.step === 'playing' ? (
                  <form onSubmit={handleRoleplaySubmit} className="flex gap-2">
                    ...
                    <button 
                      ...
                      className="bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 border border-emerald-500/50 rounded-xl px-4 py-3 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-sm font-medium whitespace-nowrap"
                    >
                      終了して<br className="sm:hidden" />評価を見る
                    </button>
                  </form>
                ) : (
                  <button 
                    onClick={() => setRoleplayState(prev => ({...prev, isOpen: false}))}
                    className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl px-4 py-3 transition-colors text-center font-medium"
                  >
                    閉じる
                  </button>
                )}
